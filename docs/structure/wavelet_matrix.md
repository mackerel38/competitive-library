...
静的配列の区間 kth、rank、頻度、前駆・後継、値の総和を扱う Wavelet Matrix です。値は座標圧縮して管理します。
...
- `range_sum(l, r)`: `[l, r)` の値の総和を返します。
- `range_sum_less(l, r, upper)`: `[l, r)` に含まれる `upper` 未満の値の総和を返します。
- `range_sum(l, r, lower, upper)`: `[l, r)` に含まれる `[lower, upper)` の値の総和を返します。
